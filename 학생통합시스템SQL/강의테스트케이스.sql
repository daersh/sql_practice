...
SELECT 
  ...
  JOIN lecture_tb c on a.lecture_id=c.id
 WHERE b.id = 20160003;
-- 7. 20160003학번 학생이 이수하지 못한 과목 조회
SELECT b.id,b.name,c.name, result_eng,result_no,complete_division,appraisal_status,appraisal_content,complete_year
  FROM course_lecture_tb a
  JOIN student_tb b on a.student_id=b.id
  JOIN lecture_tb c on a.lecture_id=c.id
 WHERE b.id = 20160003
